...
async function scrollDownAndDataGet(page, itemNumber) {
  return await page.evaluate(async (itemNumber) => {
    await util.waitFor(() => $('div.List-item').length > 0, 10)
    let itemList = $('div.List-item')
    let itemListData = []
    itemList.each((i, n) => { // n: 一个回答的所有信息
      let item = $(n)
      let answerInfoStr = item.find('.ContentItem.AnswerItem').attr('data-zop') || '{}'
      let answerInfo = JSON.parse(answerInfoStr)// 回答者信息 {itemId,authorName}

      let imgList = [] // 存储内容中图片信息
      item.find('.RichContent-inner img').each((i, n) => {
        let imgOriginUrl = $(n).attr('data-original')
        imgOriginUrl && imgList.push(imgOriginUrl)
      })
...
async function main(url) {
  ...
  await page.goto(url, { timeout: 0 })// 打开页面
  ...
  await util.asyncEach(_.times(Math.ceil(questionInfo.answerCount / 5)), async (n, i) => {
    console.log(`>>>开始滚动`, i)
    let newScrollList = await scrollDownAndDataGet(page, n) // 下拉滚动页面,获取新加载的回答模块的数据 (在页面中执行)
    // newScrollList = _.filter(newScrollList, (n, i) => n.answerUpCount > Math.sqrt(questionInfo.answerCount)) // 过滤数据
    if (_.size(newScrollList) === 0) { // 提前结束条件
      console.log(`结束`)
      return false
    }
    await util.writeArrayToJsonFile(jsonFile, newScrollList, { flag: 'a' }) // 处理数据
    console.log(`写入数据${_.size(newScrollList)}条`)

    let imageListAll = []
    _.each(newScrollList, (n, i) => {
      Array.prototype.push.apply(imageListAll, n.imgList)
    })
    console.log(`_.size(imageListAll)`, _.size(imageListAll))
    await downImgUtil.downImageList(imageListAll, questionInfo.questionId)
  })
...
// 308072414
// 309298287
main('https://www.zhihu.com/question/329062584')
